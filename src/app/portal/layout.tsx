...
export default async function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      <PortalFirebaseRedirectHandler />
      {/*
        fixed inset-0: fills the iframe viewport on mobile embeds. Using h-screen + a separate
        header made the document taller than the iframe (100vh = device height, not iframe height),
        which pushed the main content below the visible area → blank white embed.
      */}
      <div className="fixed inset-0 z-0 flex flex-col overflow-hidden bg-gray-50">
        ...
        <div className="flex min-h-0 flex-1 overflow-hidden">
          ...
          <main className="min-h-0 flex-1 overflow-y-auto p-4 pt-2 md:p-8">{children}</main>
        </div>
      </div>
    </>
  );
